...
def heatfd(xl, xr, yb, yt, M, N):
    ...
    ls = l(yb + np.arange(n+1)*k)  # Left boundary values
    rs = r(yb + np.arange(n+1)*k)  # Right boundary values
    w = np.zeros((m, n+1))   # Initialize solution matrix
    w[:,0] = f(xl + (1+np.arange(m))*h)  # Initial conditions
    for j in range(n):
        w[:,j+1]=a@w[:,j]+sigma*np.concatenate(([ls[j]],np.zeros(m-2),[rs[j]]))
    w = np.vstack((ls, w, rs))   # Attach boundary conditions
