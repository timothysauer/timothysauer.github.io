...
def houseqr(A):
    ...
    for i in range(min(n, m - 1)):
        x = A[i:m, i]  # Extract the vector x
        w = np.zeros_like(x) # Calculate the Householder vector
        w[0] = -np.sign(x[0])*np.linalg.norm(x)
        v = w - x
        H = np.eye(m)  # Form the Householder matrix
...
# Example usage:
# Q, R = houseqr(np.array([[1.,-4.],[2.,3.],[2.,2.]]))
